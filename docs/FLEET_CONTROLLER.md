...
OpenClaw visible sessions are paged in bounded groups of 200 with global/unknown categories included. Active-run and active-subagent flags override a stored terminal status. Missing activity or pagination metadata, duplicate session keys, malformed offsets, and bounds exhaustion refuse inventory. The gateway does not provide a stable session snapshot, and its public APIs omit cron executions, including work settling after a job is removed. Consequently visible unidentified work can produce an immediate blocking inventory, but an apparently idle list always refuses with `cron execution coverage is unavailable`. An empty list never enables dispatch. Resolving that gap requires a coherent read-only gateway execution aggregate covering queued, active, settling, and watcher work; listing saved cron jobs or pausing the gateway is not an idle proof.
