    ...
    def acquire(
        ...
        *,
        ...
        try:
            ...
            rows.extend(claim.__dict__ for claim in created)
            self._write(rows)
    ...
    def __enter__(self) -> "RepositoryClaimTransaction":
        try:
            for path in self.paths:
                # The repository command is authoritative for its own CLI. The
                # controller supplies only bounded values and never a shell.
                self._run(
                    (
                        "acquire",
                        "--owner",
