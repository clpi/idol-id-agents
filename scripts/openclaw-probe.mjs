...
function sanitize(value, key = "", depth = 0) {
  ...
  if (Array.isArray(value)) {
    return value.slice(0, 120).map((entry) => sanitize(entry, key, depth + 1));
  }
...
const ws = new WebSocket(gatewayUrl, {
  ...
  headers: { "User-Agent": "idol-openclaw-probe/0.1" },
});
...
async function main() {
  ...
  const hello = await call("connect", {
    ...
    locale: "en-US",
    userAgent: "idol-openclaw-probe/0.1",
  }, 45_000);
  ...
  console.log(`IDOL_OPENCLAW_PROBE_V1=${JSON.stringify(report)}`);
