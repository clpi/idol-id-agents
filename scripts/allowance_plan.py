...
providers, spends credits, redeems resets, dispatches agents, or mutates repos.
Its job is to make those later operations reviewable and fail closed.
"""
...
UTC = dt.timezone.utc
EXPIRING = {"expiring_included", "subscription_included"}
FREE = {"local", "free"}
...
def task_tokens(task: dict[str, Any]) -> int:
    ...
    return max(
        1,
        as_int(task.get("estimated_input_tokens"))
        + as_int(task.get("estimated_output_tokens")),
    )


def provider_family(provider: dict[str, Any]) -> str:
    return str(provider.get("family") or provider.get("provider") or provider.get("id") or "unknown")
...
class Window:
    ...
    def from_json(cls, raw: dict[str, Any]) -> "Window":
        ...
        return cls(
            ...
            period_seconds=(max(1, as_int(raw.get("period_seconds"))) or None),
            remaining_percent=(clamp(as_float(remaining_percent), 0.0, 100.0) if remaining_percent is not None else None),
    ...
    def urgency(self, now: dt.datetime) -> float:
        ...
        remaining_fraction = (self.remaining_percent / 100.0) if self.remaining_percent is not None else 0.5
...
class ProviderState:
    ...
    def from_json(cls, raw: dict[str, Any]) -> "ProviderState":
        windows = [Window.from_json(item) for item in raw.get("windows", []) if isinstance(item, dict)]
        state = cls(
            raw=raw,
        ...
        state.remaining_by_window = {window.name: window.remaining_tokens for window in windows}
        return state

    def role_fit(self, role: str) -> float:
        quality = self.raw.get("quality") or {}
        return clamp(as_float(quality.get(role), as_float(quality.get("default"), 0.0)), 0.0, 1.0)
    ...
    def best_window(self, tokens: int, minutes: int, now: dt.datetime) -> Window | None:
        ...
        if not candidates:
            return None
        return min(candidates, key=lambda item: item.reset_at or dt.datetime.max.replace(tzinfo=UTC))
...
def provider_rejection(task: dict[str, Any], provider: ProviderState, now: dt.datetime, paygo_approved: bool) -> str | None:
    if not provider.id or not provider.raw.get("enabled", True):
    ...
    if task.get("requires_different_family") and implementer_family and provider.family == implementer_family:
...
def candidate_score(task: dict[str, Any], provider: ProviderState, window: Window | None, now: dt.datetime) -> Candidate:
    tokens = task_tokens(task)
    minutes = as_int(task.get("estimated_minutes"), 1)
    fit = provider.role_fit(str(task.get("role") or "default"))
    completion = clamp(as_float(task.get("completion_probability"), 0.8), 0.05, 1.0)
    ...
    if provider.raw.get("premium") and not task.get("premium_required"):
        premium_penalty = 0.6
    local_bonus = 1.15 if provider.cost_class in FREE and not task.get("premium_required") else 1.0
    value = as_float(task.get("value"), 0.0)
    score = (value * fit * completion * evidence * urgency * priority * premium_penalty * local_bonus) / max(tokens, 1)
...
def conflicts(task: dict[str, Any], selected: list[Candidate]) -> str | None:
    task_paths = set(task.get("paths") or [])
    ...
    for item in selected:
        ...
        if task_paths.intersection(other.get("paths") or []):
...
def plan(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    paygo_approved = bool(payload.get("paygo_approved", False))
    providers = [ProviderState.from_json(raw) for raw in payload.get("providers", []) if isinstance(raw, dict)]
    tasks = [raw for raw in payload.get("tasks", []) if isinstance(raw, dict)]
    ...
    for task in tasks:
        ...
        for provider in providers:
            ...
            tokens = task_tokens(task)
            minutes = as_int(task.get("estimated_minutes"), 1)
            window = None if provider.cost_class in FREE else provider.best_window(tokens, minutes, now)
            candidates.append(candidate_score(task, provider, window, now))
            matched += 1
        if matched == 0:
            rejected.append({"task_id": task.get("id"), "reason": "no-eligible-provider", "provider_reasons": provider_reasons})

    candidates.sort(key=lambda item: (-item.score, item.minutes, item.tokens, str(item.task.get("id"))))
    selected: list[Candidate] = []
    selected_task_ids: set[str] = set()
    provider_parallel: dict[str, int] = {}
    for candidate in candidates:
        task_id = str(candidate.task.get("id"))
        if task_id in selected_task_ids:
            continue
        max_parallel = max(1, as_int(candidate.provider.raw.get("max_parallel"), 1))
        ...
        if candidate.window:
            ...
            if remaining < candidate.tokens:
                continue
            candidate.provider.remaining_by_window[candidate.window.name] = remaining - candidate.tokens
        candidate.provider.assigned.append(task_id)
        provider_parallel[candidate.provider.id] = provider_parallel.get(candidate.provider.id, 0) + 1
        ...
        assignments.append(
            {
                ...
                "window_reset_at": candidate.window.reset_at.isoformat() if candidate.window and candidate.window.reset_at else None,
    ...
    return {
        ...
        "warnings": [
            ...
            "Allowance telemetry with unsupported/estimated sources must not drive automatic dispatch.",
            "Every assignment still requires current live claims and an exact work order.",
            "Purchased/pay-go credits remain forbidden unless paygo_approved is explicitly true.",
        ],
    }
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema": "idol.allowance.plan.v1", "error": f"{type(exc).__name__}: {exc}"}), file=sys.stderr)
        return 2
