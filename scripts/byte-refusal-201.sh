...
path.write_text(text.replace(old_error, new_error).replace(old_quote, new_quote), encoding="utf-8")
...
grep -E 'error\{[[:space:]]*NoAllocViolation,[[:space:]]*Unsupported[[:space:]]*\}' "$src" >/dev/null 2>&1 \
...
git diff --check
git add src/codegen.zig gate/byte-emitter.sh
