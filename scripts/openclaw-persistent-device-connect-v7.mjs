...
const gatewayUrl = input.device.gatewayUrl ?? "wss://claw.idol.id";
const privateKey = crypto.createPrivateKey(input.device.privateKeyPem);
...
function signPayload(nonce, signedAt) {
  ...
  const payload = [
    ...
    d.scopes.join(","),
    String(signedAt),
    input.deviceToken ?? input.sharedToken ?? "",
    nonce ?? "",
  ].join("|");
...
function encryptBundle(bundle) {
  ...
  return {
    schema: "idol.openclaw.credential-envelope.v1",
...
const ws = new WebSocket(gatewayUrl, {
  ...
  headers: { Origin: "https://claw.idol.id", "User-Agent": "idol-openclaw-persistent-device/0.7" },
});
...
async function sendConnect(challenge = {}) {
  ...
  const auth = input.deviceToken ? { deviceToken: input.deviceToken } : { token: input.sharedToken };
  try {
    const hello = await request("connect", {
      ...
      auth,
      locale: "en-US",
      userAgent: "idol-openclaw-persistent-device/0.7",
      device: {
        id: input.device.deviceId,
        publicKey: input.device.publicKey,
        signature: signPayload(nonce, signedAt),
        signedAt,
        nonce,
      },
    });
    ...
    const report = {
      schema: "idol.openclaw.persistent-connect.v1",
      observedAt: new Date().toISOString(),
      outcome: "connected",
      deviceId: input.device.deviceId,
      deviceIdPrefix: input.device.deviceId.slice(0, 16),
    ...
    if (hello?.auth?.deviceToken) {
      fs.writeFileSync(envelopeFile, JSON.stringify(encryptBundle({
        schema: "idol.openclaw.persistent-credential.v1",
        gatewayUrl,
        device: input.device,
        deviceToken: hello.auth.deviceToken,
        scopes: hello.auth.scopes,
        recoveryScope: hello.auth.recoveryScope,
    ...
    fs.writeFileSync(resultFile, JSON.stringify({
      schema: "idol.openclaw.persistent-connect.v1",
      observedAt: new Date().toISOString(),
      outcome: error?.code === "PAIRING_REQUIRED" ? "pairing-required" : "connect-failed",
      deviceId: input.device.deviceId,
      deviceIdPrefix: input.device.deviceId.slice(0, 16),
    ...
    fs.writeFileSync(envelopeFile, JSON.stringify(encryptBundle({
      schema: "idol.openclaw.pending-device.v1",
      gatewayUrl,
      device: input.device,
      sharedToken: input.sharedToken,
      pairing: error?.details,
...
ws.once("error", (error) => {
  if (!finished) fs.writeFileSync(resultFile, JSON.stringify({ schema: "idol.openclaw.persistent-connect.v1", outcome: "transport-error", error: safeError(error) }, null, 2));
});
ws.once("close", (code, reason) => {
  if (!finished) fs.writeFileSync(resultFile, JSON.stringify({ schema: "idol.openclaw.persistent-connect.v1", outcome: "socket-closed", error: safeError({ code: "SOCKET_CLOSED", message: "gateway closed socket", closeCode: code, closeReason: reason.toString("utf8") }) }, null, 2));
});
