...
if (!hermesPassword || !chromeBin) throw new Error("required encrypted Hermes runtime inputs are absent");
...
async function openTerminal() {
  const browser = await chromium.launch({ executablePath: chromeBin, headless: true, args: ["--no-sandbox", "--disable-dev-shm-usage", "--disable-gpu"] });
  ...
  try {
    ...
    const state = await page.evaluate(async () => {
      ...
      while (Date.now() < deadline && (!S?.session?.session_id || !S?.session?.workspace)) await new Promise((resolve) => setTimeout(resolve, 250));
    ...
    await page.evaluate(async ({ sessionId }) => {
      await api("/api/terminal/start", { method: "POST", body: JSON.stringify({ session_id: sessionId, rows: 40, cols: 160, restart: false }) });
...
async function main() {
  const report = { schema: "idol.hermes.claw-assets-repair.v1", submitted: false, waitedMs: 0, semanticOutcomeClaimed: false };
  let hermes;
  try {
    hermes = await openTerminal();
    const command = encodeCommand(repair);
    await hermes.page.evaluate(async ({ sessionId, data }) => {
      await api("/api/terminal/input", { method: "POST", body: JSON.stringify({ session_id: sessionId, data }) });
    ...
    if (hermes) {
      await hermes.page.evaluate(async ({ sessionId }) => {
        await api("/api/terminal/close", { method: "POST", body: JSON.stringify({ session_id: sessionId }) });
