...
class ManagerTests(unittest.TestCase):
    ...
    def test_inventory_failure_blocks_every_work_order(self) -> None:
        temporary, config_path, head, cancel_log = self.fixture()
        with temporary:
            raw = json.loads(config_path.read_text())
            inventory_path = Path(raw["inventory"]["command"][1])
            inventory_path.write_text("raise SystemExit(1)\n")
            controller = ManagedFleetController(config_path=config_path)
            result = controller.run_once()
            self.assertFalse(result.plan.assignments)
            reasons = [reason for row in result.plan.rejections for reason in row.reasons]
            self.assertIn("live-inventory-unavailable", reasons)
            kinds = [row["kind"] for row in controller.journal.verify()]
            self.assertIn("fleet.inventory.refused", kinds)

    def test_unidentified_running_session_blocks_dispatch(self) -> None:
        temporary, config_path, head, cancel_log = self.fixture(unidentified=True)
        with temporary:
            controller = ManagedFleetController(config_path=config_path)
            result = controller.run_once()
            self.assertFalse(result.plan.assignments)
